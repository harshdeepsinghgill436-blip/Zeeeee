...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
const DAILY_LIMIT = 300      // per Brevo account
const SEND_DELAY_MS = 1200   // 1.2s gap between sends → ~50/min, well under Brevo limits
// ─────────────────────────────────────────────────────────────────────────────

const SEQUENCES = [
  { id: 1, subject: "Your next client posted this morning", body: `Most founders pay for lists that are 6 months old and wonder why nobody replies.\n\nLeadSynth scans Reddit, LinkedIn, Upwork, and 40+ sources in real time — surfacing people actively posting right now asking for your exact service. Verified contact. Live intent. One click export.\n\nYou're reaching out after the deal is already gone.\n\nSee who's buying today — 10 leads free, no card needed.\n→ https://lead-synth-pro.vercel.app/`, followups: [{ subject: "Still sending cold emails?", body: `LeadSynth shows you people who posted today asking for your service. Verified contact attached.\n\nEvery day you wait, someone else replies first.\n→ https://lead-synth-pro.vercel.app/` }, { subject: "The deal you didn't know existed", body: `Someone in your niche posted on Reddit this morning looking for exactly what you sell. You didn't see it. Your competitor did.\n\n10 free leads. See what you're missing.\n→ https://lead-synth-pro.vercel.app/` }, { subject: "Your pipeline is a graveyard", body: `Old data. Stale contacts. People who stopped caring months ago.\n\nLeadSynth surfaces buyers who are active right now — not six months ago.\n\nTest it free. No card.\n→ https://lead-synth-pro.vercel.app/` }, { subject: "Last one from me", body: `I won't keep following up after this.\n\nBut if closing warm buyers instead of chasing cold lists sounds worth 2 minutes — try it free.\n→ https://lead-synth-pro.vercel.app/` }, { subject: "One question before I go", body: `Are you still manually prospecting?\n\nBecause right now someone in your niche is actively looking for you. LeadSynth finds them. 10 free leads to prove it.\n→ https://lead-synth-pro.vercel.app/` }] },
...
serve(async (req) => {
  const headers = {
    ...
    'Access-Control-Allow-Headers': 'authorization, content-type, x-brevo-accounts',
  }

  if (req.method === 'OPTIONS') return new Response(null, { headers })

  // ── Load Brevo accounts ──────────────────────────────────────────────────────
  // Priority 1: x-brevo-accounts header (from app when device is on)
  // Priority 2: Supabase secrets BREVO_KEY_1..10 / BREVO_EMAIL_1..10 (for device-off cron)
  ...
  if (BREVO_ACCOUNTS.length === 0) {
    for (let i = 1; i <= 10; i++) {
      const key = Deno.env.get(`BREVO_KEY_${i}`) || ''
      const email = Deno.env.get(`BREVO_EMAIL_${i}`) || ''
      if (key && email) BREVO_ACCOUNTS.push({ key, email })
    }
  }

  if (BREVO_ACCOUNTS.length === 0) {
    return new Response(
      JSON.stringify({ ok: false, error: 'No Brevo accounts. Add BREVO_KEY_1 + BREVO_EMAIL_1 secrets in Supabase Dashboard → Edge Functions → send-emails → Secrets' }),
      { status: 400, headers }
    )
  }
  ...
  try {
    ...
    // Build key lookup map (for device-off case where secrets override stored key)
    const brevoKeyMap: Record<string, string> = {}
    for (const acc of BREVO_ACCOUNTS) brevoKeyMap[acc.email] = acc.key

    // ── STEP 1: Follow-ups due ── (FIX: excludes deleted leads)
    ...
    for (const lead of (followupLeads || [])) {
      const senderEmail = lead.brevo_sender
      const brevoKey = brevoKeyMap[senderEmail] || lead.brevo_key
      if (!brevoKey) continue
      if ((dailyCounts[senderEmail] || 0) >= DAILY_LIMIT) continue
      const seq = SEQUENCES.find(s => s.id === lead.sequence_id)
      if (!seq) continue
      const followup = seq.followups[lead.current_step - 1]
      if (!followup) continue
      try {
        await sendEmail(brevoKey, senderEmail, lead.email, followup.subject, followup.body)
        const nextStep = lead.current_step + 1
        const isDone = nextStep > seq.followups.length
        await supabase.from('email_logs').insert({ lead_id: lead.id, step: lead.current_step, subject: followup.subject, sent_at: now.toISOString(), status: 'sent', brevo_sender: senderEmail })
        ...
        dailyCounts[senderEmail] = (dailyCounts[senderEmail] || 0) + 1
        fired.push(lead.email)
        await sleep(SEND_DELAY_MS) // FIX: rate-limit guard
      } catch (e: any) {
        errors.push(`followup ${lead.email}: ${e.message}`)
        console.error(`Followup failed ${lead.email}:`, e)
      }
    }

    // ── STEP 2: Pending main emails ── (FIX: 'deleted' leads have status='deleted', not 'pending', but guard anyway)
    ...
    for (const lead of (newLeads || [])) {
      ...
      if (!seq) continue
      try {
        await sendEmail(brevoKey, senderEmail, lead.email, seq.subject, seq.body)
        await supabase.from('email_logs').insert({ lead_id: lead.id, step: 0, subject: seq.subject, sent_at: now.toISOString(), status: 'sent', brevo_sender: senderEmail })
        ...
        errors.push(`main ${lead.email}: ${e.message}`)
        console.error(`Main email failed ${lead.email}:`, e)
      }
    }

    return new Response(JSON.stringify({ ok: true, fired: fired.length, emails: fired, errors }), { headers })
  } catch (e: any) {
